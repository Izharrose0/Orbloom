...
function PulsarRing() {
  ...
  useFrame((state, delta) => {
    ...
    const { stage, mass, genome } = useGameStore.getState();
    const elapsed = state.clock.elapsedTime;
    uniforms.uTime.value = elapsed;
    uniforms.uColor.value.setHSL(((genome.hueGlow + stage.paletteShift) % 360) / 360, 0.85, 0.65);
    ref.current.scale.setScalar(visualScaleForMass(mass));
...
function PlasmaCorona() {
  const ref = useRef<THREE.Mesh>(null);
  const uniforms = useMemo(() => ({
    uTime: { value: 0 }, uPulse: { value: 0 }, uColor: { value: new THREE.Color('#ffae66') },
  }), []);

  useFrame((state, delta) => {
    if (!ref.current) return;
    const { mass, pulseIntensity, genome, stage } = useGameStore.getState();
    const elapsed = state.clock.elapsedTime;
    uniforms.uTime.value = elapsed;
    uniforms.uPulse.value = pulseIntensity;
    uniforms.uColor.value.setHSL((((genome.hueGlow + stage.paletteShift + 25) % 360)) / 360, 0.9, 0.62);
    const s = visualScaleForMass(mass) * (1.55 + 0.04 * Math.sin(elapsed * 0.7));
      ...
      <shaderMaterial
        ...
        fragmentShader={`
          ...
          void main() {
            ...
            float a = f * (0.32 + uPulse * 0.4) * flicker;
...
function Tendrils() {
  ...
  useFrame((state, delta) => {
    if (!ref.current) return;
    const { mass } = useGameStore.getState();
    const elapsed = state.clock.elapsedTime;
    ref.current.scale.setScalar(visualScaleForMass(mass));
    ref.current.rotation.y += delta * 0.18;
    ref.current.rotation.x = Math.sin(elapsed * 0.3) * 0.08;
      ...
      {Array.from({ length: TENDRIL_COUNT }).map((_, i) => {
        ...
        return (
          <mesh key={i}>
            ...
            <meshBasicMaterial color="#cfa8ff" transparent opacity={0.45} blending={THREE.AdditiveBlending} depthWrite={false} />
          </mesh>
        );
      })}
    </group>
...
function AccretionDisc() {
  const ref = useRef<THREE.Mesh>(null);
  const uniforms = useMemo(() => ({
    uTime: { value: 0 }, uColor: { value: new THREE.Color('#ff66cc') },
  }), []);

  useFrame((state, delta) => {
    if (!ref.current) return;
    const { mass, genome, stage } = useGameStore.getState();
    uniforms.uTime.value = state.clock.elapsedTime;
    uniforms.uColor.value.setHSL(((genome.hueGlow + stage.paletteShift) % 360) / 360, 0.85, 0.6);
    const s = visualScaleForMass(mass) * 2.8;
    ref.current.scale.set(s, s, 1);
    ref.current.rotation.z += delta * 0.25;
  });

  return (
    <mesh ref={ref} rotation={[Math.PI / 2.05, 0, 0]}>
...
export default function StageDecorations() {
  ...
  return (
    <group>
      {stage.ringEnabled && <PulsarRing />}
      {stage.coronaEnabled && <PlasmaCorona />}
      {stage.tendrilsEnabled && <Tendrils />}
      {stage.discEnabled && <AccretionDisc />}
    </group>
  );
}
